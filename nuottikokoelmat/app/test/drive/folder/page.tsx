...
export default function Home() {
  ...
  const getFileList = async (returnDevelopmentTree: boolean) => {
    ...
    const response = await fetch(
      `/api/archive/${archiveId}/manage/drive/folder/${folderId}?developmentTree=${returnDevelopmentTree}`
    )
    if (response.ok) {
    ...
    <main className='flex flex-col items-center p-2 gap-4 h-full justify-start h-screen'>
      <div className='justify-start items-center gap-4 flex-col flex'>
        ...
        <NpInput
          ...
          placeholder='Kansion id'
        />
        <NpInput
          label='Akiston id'
          value={archiveId}
          onChange={(e) => setArchiveId(e.target.value)}
          placeholder='Arkiston id'
        />
      </div>

      <NpButton disabled={isLoading} onClick={() => getFileList(true)}>
        Hae development tree
      </NpButton>
      <NpButton disabled={isLoading} onClick={() => getFileList(false)}>
        Hae kansion tiedostotlistaus
      </NpButton>
