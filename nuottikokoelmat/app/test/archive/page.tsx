...
export default function Home() {
	...
	const saveSongs = async (id: string) => {
		const songs: Song[] = JSON.parse(songsJson).map((s: SongNoArchiveId) => ({ ...s, archiveId: id as unknown as mongoose.Types.ObjectId }))
		const response = await fetch(`/api/archive/${id}/manage/songs`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
	...
	return (
		<main className='flex flex-col items-center p-2 gap-4 h-full justify-start h-screen'>
			<div className='justify-start items-center gap-4 flex-col flex'>
				...
				<NpInput label='Vierailijoiden salasana' value={visitorPassword} onChange={(e) => setVisitorPassword(e.target.value)} />
