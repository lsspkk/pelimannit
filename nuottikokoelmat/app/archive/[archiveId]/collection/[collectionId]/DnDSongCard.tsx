...
export const DnDSongCard = (
	{ song, index, id, onLoadPdf }: { song: Song; index: number; id: string; onLoadPdf: (index: number) => void },
) => {
	const { listeners, setNodeRef, transform, transition } = useSortable({ id })
	const router = useRouter()

	const style = transform ? { transform: CSS.Transform.toString(transform), transition, background: 'rgba(235, 235, 235, 0.9)' } : undefined
	return (
		<div ref={setNodeRef} className='w-full' style={style}>
			<NpButtonCard>
				<div className='flex-col w-10/12 flex' onClick={() => onLoadPdf(index)}>
