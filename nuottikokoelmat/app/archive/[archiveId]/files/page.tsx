...
import { useFileMap } from '@/stores/fileContext'
...
export default function Home({ params }: { params: { archiveId: string } }) {
	...
	const onUploadDirectory = async (event: React.ChangeEvent<HTMLInputElement>) => {
		if (event.target.files) {
			const { fileMap: newFileMap, missingSongs: newMissingSongs } = getFileMap(songs || [], Array.from(event.target.files))
		...
		<NpMain title='Tiedostot'>
			...
			<div className='flex flex-col gap-2 w-full items-start'>
				...
				<div className='flex gap-4 w-full justify-between text-sm items-end'>
					<div className='text-sm'>Ladattu: {fileMap.size}, puuttuu: {missingSongs.length}</div>
					<NpButton onClick={() => hiddenFileInputRef.current?.click()}>{fileMap.size > 0 ? 'Päivitä' : 'Lataa'}</NpButton>
				</div>

				{missingSongs.length > 0 && (
					<div className='flex flex-col gap-2 w-full items-start'>
						<NpSubTitle>Puuttuvat tiedostot</NpSubTitle>
						{missingSongs.map((s) => <div key={s._id.toString() + '+missing'}>{s.path + s.songname}</div>)}
					</div>
				)}
				...
				{fileMap.size > 0 && (
					<div className='flex flex-col gap-2 w-full items-start'>
						...
						{Array.from(fileMap.values()).sort(fileNameSorter).map((value, index) => (
							<div key={showFileName(value) + '+file'} className='text-xs'>{index + 1} {showFileName(value)}</div>
						))}
					</div>
				)}
			</div>
