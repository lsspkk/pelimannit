...
export const PdfAlternativeFileView = ({
  ...
  useEffect(() => {
    fetchPdf(songs[index], alternativeUrl)
      .then((blob) => {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      const container = window.document.querySelector('.overflow-y-scroll') as HTMLElement
      if (!container) return

      if (event.key === 'ArrowDown') {
        if (container.scrollTop + container.clientHeight < container.scrollHeight) {
          // Scroll one screen or to the bottom
          container.scrollBy({ top: container.clientHeight, behavior: 'smooth' })
        } else {
          // At the bottom, move to the next page or file
          if (!lastPage) {
        ...
        if (container.scrollTop > 0) {
          // Scroll one screen or to the top
          container.scrollBy({ top: -container.clientHeight, behavior: 'smooth' })
        } else {
          // At the top, move to the previous page or file
          if (!firstPage) {
            setCurrentPage((prev) => prev - 1)
          } else if (index > 0) {
            setIndex((prev) => prev - 1)
  ...
  }, [currentPage, firstPage, lastPage, index, songs.length])
