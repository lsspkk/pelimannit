...
const apiHandler = async (req: NextApiRequest, res: NextApiResponse, collectionId: string): Promise<void> => {
	if (req.method === 'PUT') {
		...
		const response = await secureFetch(`/api/v1/collection/${collectionId}`, { method: 'DELETE' })
		const json = await response.json()
		res.status(response.status).json(json)
	} else {
...
const mongoHandler = async (req: NextApiRequest, res: NextApiResponse, collectionId: string): Promise<void> => {
	...
	if (req.method === 'GET') {
		...
		const collections: Array<Collection> = await CollectionModel.find({ _id: collectionId }).exec()
